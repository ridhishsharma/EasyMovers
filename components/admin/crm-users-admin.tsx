...
export function CrmUsersAdmin({ supabaseUrl, publishableKey }: { supabaseUrl: string; publishableKey: string }) {
  ...
  const roleChoices = <fieldset><legend>CRM roles</legend><p>Permissions are inherited from the selected roles.</p>{roles.map(role => <label key={role.code}><input type="checkbox" checked={roleCodes.includes(role.code)} onChange={() => toggleRole(role.code)} /><span><strong>{role.name}</strong><small>{role.description}</small></span></label>)}</fieldset>;
  ...
  return <main className={styles.page}>
    <header className={styles.heading}><div><p>SECURITY &amp; ACCESS</p><h1>Office users</h1><span>Assign responsibilities without giving every employee unrestricted access.</span></div><button className={styles.primary} onClick={beginCreate}>Add office user</button></header>
