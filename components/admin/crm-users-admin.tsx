...
type User = {
  ...
  officeInvitedAt: string | null; officeInvitationAcceptedAt: string | null; officePasswordSetAt: string | null;
  officeFirstLoginAt: string | null; officeLastLogoutAt: string | null;
...
function lifecycleStatus(user: User) {
  if (!user.isActive) return { label: "Suspended", className: styles.suspendedStatus };
  if (user.officeFirstLoginAt) return { label: "Active", className: styles.activeStatus };
  if (user.officePasswordSetAt) return { label: "Password ready", className: styles.readyStatus };
  if (user.officeInvitationAcceptedAt) return { label: "Password pending", className: styles.acceptedStatus };
  return { label: "Invitation pending", className: styles.pendingStatus };
}
...
export function CrmUsersAdmin({ supabaseUrl, publishableKey }: { supabaseUrl: string; publishableKey: string }) {
  ...
  <button className={styles.primary} onClick={beginCreate}>
    ...
    <div className={styles.workspace}>
      <section className={styles.list} aria-label="Office users"><div className={styles.listTitle}><strong>{users.length} office user{users.length === 1 ? "" : "s"}</strong><span>Maximum 100</span></div><div className={styles.scroll}>{users.map(user => { const status = lifecycleStatus(user); return <button key={user.id} className={selected?.id === user.id ? styles.selectedUser : styles.user} onClick={() => choose(user)}><span><strong>{user.fullName}</strong><small className={status.className}>{status.label}</small></span><span>{user.email || user.mobile}</span><span className={styles.roleBadges}>{user.crmRoleAssignments.length ? user.crmRoleAssignments.map(item => <small key={item.role.code} className={isAdministratorRole(item.role.code) ? styles.administratorBadge : styles.operationalBadge}>{item.role.name}</small>) : <small className={styles.noRoleBadge}>No active CRM role</small>}</span></button>; })}</div></section>
      <section className={styles.detail} aria-label="Office user access"><div className={styles.detailScroll}>{creating ? <form onSubmit={invite}><div className={styles.person}><div><h2>Add office user</h2><p>A secure password-setup invitation will be emailed to the employee.</p></div><button type="button" className={styles.close} onClick={() => setCreating(false)}>×</button></div><div className={styles.fields}><label>Full name<input required maxLength={100} value={fullName} onChange={event => setFullName(event.target.value)} /></label><label>Official email<input required type="email" maxLength={254} value={email} onChange={event => setEmail(event.target.value)} /></label><label>Mobile number<input required inputMode="numeric" maxLength={10} value={mobile} onChange={event => setMobile(event.target.value.replace(/\D/g, "").slice(0, 10))} /></label></div>{roleChoices}<div className={styles.actionFooter}><button className={styles.primary} disabled={busy || !roleCodes.length}>{busy ? "Sending invitation…" : "Create user & send invitation"}</button></div></form> : !selected ? <div className={styles.empty}><h2>Select an office user</h2><p>Review account status and assigned CRM roles.</p></div> : <form onSubmit={save}><div className={styles.person}><div><h2>{selected.fullName}</h2><p>{selected.email || "No email"} · {selected.mobile}</p></div><span className={lifecycleStatus(selected).className}>{lifecycleStatus(selected).label}</span></div><dl><dt>Legacy account role</dt><dd>{selected.role.replaceAll("_", " ")}</dd><dt>Invitation sent</dt><dd>{dateTime(selected.officeInvitedAt)}</dd><dt>Invitation accepted</dt><dd>{dateTime(selected.officeInvitationAcceptedAt)}</dd><dt>Password configured</dt><dd>{dateTime(selected.officePasswordSetAt)}</dd><dt>First CRM login</dt><dd>{dateTime(selected.officeFirstLoginAt)}</dd><dt>Last login</dt><dd>{dateTime(selected.lastLogin)}</dd><dt>Last logout</dt><dd>{dateTime(selected.officeLastLogoutAt)}</dd></dl>{roleChoices}<label className={styles.statusControl}><input type="checkbox" checked={active} disabled={selected.id === currentUserId} onChange={event => setActive(event.target.checked)} /><span><strong>Office access active</strong><small>{selected.id === currentUserId ? "You cannot deactivate your own account." : "Turn off to block CRM access immediately."}</small></span></label><div className={styles.actionFooter}><button className={styles.primary} disabled={busy || !roleCodes.length}>{busy ? "Saving…" : "Save access"}</button></div></form>}</div></section>
    </div>
  </main>;
}
