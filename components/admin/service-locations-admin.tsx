...
type LocationDetail = LocationSummary & { serviceablePostalCodes: string[]; operationsContactName: string | null; operationsContactMobile: string | null; operationsContactEmail: string | null; approvedAt: string | null; suspensionReason: string | null; services: LocationService[] };
...
export function ServiceLocationsAdmin({ supabaseUrl, publishableKey }: { supabaseUrl: string; publishableKey: string }) {
  ...
  const [locationForm, setLocationForm] = useState({ code: "", city: "", state: "", pins: "", contactName: "", contactMobile: "", contactEmail: "", plannedLaunchAt: "" });
  ...
  useEffect(() => { if (signedIn) void loadLocations(1); }, [signedIn, loadLocations]);

  function resetLocationForm(location?: LocationDetail) {
    setLocationForm(location ? {
      code: location.code, city: location.city, state: location.state, pins: location.serviceablePostalCodes.join(", "),
    ...
    } : { code: "", city: "", state: "", pins: "", contactName: "", contactMobile: "", contactEmail: "", plannedLaunchAt: "" });
  }

  function beginCreate() { resetLocationForm(); setCreating(true); setEditing(false); setSelected(null); setReadiness(null); setMessage(""); }
  ...
  async function saveLocation(event: FormEvent) {
    ...
    setBusy(true); setMessage("");
    const body = { code: locationForm.code || undefined, city: locationForm.city, state: locationForm.state, serviceablePostalCodes: locationForm.pins.split(/[\s,]+/).filter(Boolean), operationsContactName: locationForm.contactName || null, operationsContactMobile: locationForm.contactMobile || null, operationsContactEmail: locationForm.contactEmail || null, plannedLaunchAt: locationForm.plannedLaunchAt || null };
  ...
  const locationFormPanel = <form className={styles.locationForm} onSubmit={saveLocation}>
    ...
    <div className={styles.formGrid}><label>City<input required maxLength={80} value={locationForm.city} onChange={event => setLocationForm(value => ({ ...value, city: event.target.value }))} /></label><label>State<input required maxLength={80} value={locationForm.state} onChange={event => setLocationForm(value => ({ ...value, state: event.target.value }))} /></label><label>Location code<input maxLength={50} placeholder="Generated when blank" value={locationForm.code} onChange={event => setLocationForm(value => ({ ...value, code: event.target.value.toUpperCase().replace(/[^A-Z0-9-]/g, "") }))} /></label><label>Planned launch<input type="date" value={locationForm.plannedLaunchAt} onChange={event => setLocationForm(value => ({ ...value, plannedLaunchAt: event.target.value }))} /></label><label className={styles.wide}>Serviceable PIN codes<input placeholder="462011, 462016" value={locationForm.pins} onChange={event => setLocationForm(value => ({ ...value, pins: event.target.value.replace(/[^0-9,\s]/g, "") }))} /></label><label>Operations contact<input maxLength={100} value={locationForm.contactName} onChange={event => setLocationForm(value => ({ ...value, contactName: event.target.value }))} /></label><label>Operations mobile<input inputMode="numeric" maxLength={10} value={locationForm.contactMobile} onChange={event => setLocationForm(value => ({ ...value, contactMobile: event.target.value.replace(/\D/g, "").slice(0, 10) }))} /></label><label className={styles.wide}>Operations email<input type="email" maxLength={254} value={locationForm.contactEmail} onChange={event => setLocationForm(value => ({ ...value, contactEmail: event.target.value }))} /></label></div>
  ...
  return <main className={styles.page}>
    ...
    {message && <p className={message.includes("success") || message.includes("saved") || message.includes("created") || message.includes("updated") || message.includes("ready") || message.includes("suspended") ? styles.success : styles.error} role="status">{message}</p>}
    <div className={styles.workspace}>
      ...
      <section className={styles.detailPanel}>
        {creating || editing ? locationFormPanel : !selected ? <div className={styles.emptyDetail}><h2>Select a service location</h2><p>Review its launch status, vendor capacity and individual services.</p></div> : <div className={styles.detailScroll}>
          ...
          <dl className={styles.locationMeta}><dt>PIN coverage</dt><dd>{selected.serviceablePostalCodes.length ? selected.serviceablePostalCodes.join(", ") : "Not configured"}</dd><dt>Planned launch</dt><dd>{dateTime(selected.plannedLaunchAt)}</dd><dt>Operations contact</dt><dd>{selected.operationsContactName || "Not assigned"}{selected.operationsContactMobile ? ` · +91 ${selected.operationsContactMobile}` : ""}</dd>{selected.suspensionReason && <><dt>Suspension reason</dt><dd>{selected.suspensionReason}</dd></>}</dl>
