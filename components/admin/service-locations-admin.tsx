...
export function ServiceLocationsAdmin({ supabaseUrl, publishableKey }: { supabaseUrl: string; publishableKey: string }) {
  ...
  async function addAllStandardServices() {
    ...
    if (!missing.length) { setMessage("All standard within-city services are already configured."); return; }
    ...
    try {
      ...
      setMessage(`${data.createdCount} standard service${data.createdCount === 1 ? "" : "s"} added as Draft.`);
    } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to add the standard services."); setBusy(false); }
  }
