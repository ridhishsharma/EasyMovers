...
type CapacityVendor = { id: string; vendorCode: string; companyName: string };
type ReadinessService = { id: string; scope: Scope; serviceType: ServiceType; status: ServiceStatus; verifiedVendors: number; minimumVerifiedVendors: number; shortage: number; ready: boolean; qualifyingVendors: CapacityVendor[]; additionalVendors: number };
type Readiness = { locationStatus: LocationStatus; services: ReadinessService[]; readyServices: number; capacityGaps: ReadinessService[]; blockers: ReadinessService[] };
...
const isSuccessMessage = (value: string) => /success|saved|created|updated|ready|suspended|verified|added/i.test(value);
...
export function ServiceLocationsAdmin({ supabaseUrl, publishableKey }: { supabaseUrl: string; publishableKey: string }) {
  ...
  const [serviceMessage, setServiceMessage] = useState<{ tone: "success" | "error" | "info"; text: string } | null>(null);
  ...
  async function saveService(event: FormEvent) {
    ...
    setBusy(true); setMessage(""); setServiceMessage(null);
    try {
      await api(`/api/admin/service-locations/${selected.id}/services`, { method: "PUT", body: JSON.stringify(serviceForm) });
      await openLocation(selected.id); await loadLocations(pagination.page); setServiceMessage({ tone: "success", text: `${label(serviceForm.serviceType)} configuration saved.` });
    } catch (error) { setServiceMessage({ tone: "error", text: error instanceof Error ? error.message : "Unable to save the service." }); setBusy(false); }
  }

  async function addAllStandardServices() {
    if (!selected || busy) return;
    const configured = new Set(selected.services.filter(service => service.scope === "WITHIN_CITY").map(service => service.serviceType));
    const missing = serviceTypes.filter(serviceType => !configured.has(serviceType));
    if (!missing.length) { setServiceMessage({ tone: "info", text: "All standard within-city services are already configured." }); return; }
    if (!window.confirm(`Add ${missing.length} remaining within-city services as Draft? You can adjust each service before activation.`)) return;
    setBusy(true); setMessage(""); setServiceMessage(null);
    try {
      const data = await api(`/api/admin/service-locations/${selected.id}/services`, { method: "POST", body: JSON.stringify({ serviceTypes: missing }) });
      setSelected(current => current ? { ...current, services: data.services } : current);
      await openLocation(selected.id); await loadLocations(pagination.page);
      setServiceMessage({ tone: data.createdCount ? "success" : "info", text: data.createdCount ? `${data.createdCount} standard service${data.createdCount === 1 ? "" : "s"} added as Draft.` : "No services were added because every standard service already exists." });
    } catch (error) { setServiceMessage({ tone: "error", text: error instanceof Error ? error.message : "Unable to add the standard services." }); setBusy(false); }
  }
  ...
  const selectedServiceCapacity = readiness?.services.find(service => service.scope === serviceForm.scope && service.serviceType === serviceForm.serviceType);
  const canSetServiceReady = Boolean(selectedServiceCapacity?.ready);
  const canMarkLocationReady = Boolean(readiness && readiness.readyServices > 0 && readiness.blockers.length === 0);
  const serviceMessageClass = serviceMessage?.tone === "success" ? styles.serviceSuccess : serviceMessage?.tone === "error" ? styles.serviceError : styles.serviceInfo;
    ...
    <div className={styles.workspace}>
      ...
      <section className={styles.detailPanel}>
        {creating || editing ? locationFormPanel : !selected ? <div className={styles.emptyDetail}><h2>Select a service location</h2><p>Review its launch status, vendor capacity and individual services.</p></div> : <div className={styles.detailScroll}>
          ...
          <div className={styles.summaryCards}><article><span>Configured services</span><strong>{selected.services.length}</strong></article><article><span>Ready services</span><strong>{readiness?.readyServices ?? 0}</strong></article><article className={readiness?.capacityGaps.length ? styles.warningCard : styles.goodCard}><span>Services needing vendors</span><strong>{readiness?.capacityGaps.length ?? 0}</strong><small>{readiness?.capacityGaps.length ? "Capacity requirement not met" : "All service capacity requirements met"}</small></article></div>
          ...
          {readiness?.blockers.length ? <div className={styles.blockers}><strong>Launch blockers</strong>{readiness.blockers.map(item => <span key={item.id}>{label(item.serviceType)} ({label(item.scope)}): add {item.shortage} more verified vendor{item.shortage === 1 ? "" : "s"} ({item.verifiedVendors} of {item.minimumVerifiedVendors} available)</span>)}</div> : null}
          <section className={styles.services}><div className={styles.sectionTitle}><div><h3>Service matrix</h3><p>Within-city is a geographic scope; each moving service is configured independently.</p></div></div>{selected.services.length ? <div className={styles.serviceTable}><table><thead><tr><th>Service</th><th>Scope</th><th>Fulfilment</th><th>Capacity</th><th>Status</th><th></th></tr></thead><tbody>{selected.services.map(service => { const capacity = readiness?.services.find(item => item.id === service.id); return <tr key={service.id}><td><strong>{label(service.serviceType)}</strong><small>{service.surveyRequired ? "Survey required" : "No mandatory survey"}{service.instantPricingAvailable ? " · Instant price" : ""}</small></td><td>{label(service.scope)}</td><td>{label(service.fulfilmentMode)}</td><td>{capacity ? <div className={styles.capacity}><strong className={capacity.ready ? styles.capacityReady : styles.capacityShort}>{capacity.verifiedVendors}/{capacity.minimumVerifiedVendors}</strong><small>{capacity.ready ? "Capacity met" : `${capacity.shortage} vendor${capacity.shortage === 1 ? "" : "s"} required`}</small>{capacity.qualifyingVendors.length ? <details><summary>View vendors</summary>{capacity.qualifyingVendors.map(vendor => <span key={vendor.id}>{vendor.companyName}<small>{vendor.vendorCode}</small></span>)}{capacity.additionalVendors > 0 && <em>+{capacity.additionalVendors} more</em>}</details> : <small>No qualifying active vendor</small>}</div> : "—"}</td><td><span className={`${styles.badge} ${styles[service.status]}`}>{label(service.status)}</span></td><td>{capabilities.canManage && selected.status !== "ACTIVE" && <button className={styles.textButton} onClick={() => editService(service)}>Edit</button>}</td></tr>; })}</tbody></table></div> : <p className={styles.emptyServices}>No services configured. Add the first service below.</p>}</section>
          {capabilities.canManage && selected.status !== "ACTIVE" && <form className={styles.serviceForm} onSubmit={saveService}><div className={styles.standardSetup}><div><strong>Standard within-city setup</strong><span>Add every service not yet configured using safe Draft defaults. Existing services will not be changed.</span></div><button type="button" className={styles.secondary} disabled={busy || serviceTypes.every(serviceType => selected.services.some(service => service.scope === "WITHIN_CITY" && service.serviceType === serviceType))} onClick={() => void addAllStandardServices()}>Add all remaining services</button></div>{serviceMessage && <p className={styles.serviceNotice + " " + serviceMessageClass} role="status">{serviceMessage.text}</p>}<h3>Configure one service</h3><div className={styles.serviceGrid}><label>Scope<select value={serviceForm.scope} onChange={event => setServiceForm(value => ({ ...value, scope: event.target.value as Scope }))}>{scopes.map(item => <option key={item} value={item}>{label(item)}</option>)}</select></label><label>Service<select value={serviceForm.serviceType} onChange={event => setServiceForm(value => ({ ...value, serviceType: event.target.value as ServiceType }))}>{serviceTypes.map(item => <option key={item} value={item}>{label(item)}</option>)}</select></label><label>Fulfilment<select value={serviceForm.fulfilmentMode} onChange={event => { const mode = event.target.value as FulfilmentMode; setServiceForm(value => ({ ...value, fulfilmentMode: mode, instantPricingAvailable: mode === "INSTANT_RATE" && value.instantPricingAvailable })); }}>{fulfilmentModes.map(item => <option key={item} value={item}>{label(item)}</option>)}</select></label><label>Readiness<select value={serviceForm.status} onChange={event => setServiceForm(value => ({ ...value, status: event.target.value as "DRAFT" | "READY" }))}><option value="DRAFT">Draft</option><option value="READY" disabled={!canSetServiceReady}>Ready — requires verified vendor capacity</option></select></label><label>Minimum vendors<input type="number" min={1} max={100} value={serviceForm.minimumVerifiedVendors} onChange={event => setServiceForm(value => ({ ...value, minimumVerifiedVendors: Number(event.target.value) }))} /></label><label className={styles.check}><input type="checkbox" checked={serviceForm.surveyRequired} onChange={event => setServiceForm(value => ({ ...value, surveyRequired: event.target.checked }))} /><span>Survey required</span></label><label className={styles.check}><input type="checkbox" disabled={serviceForm.fulfilmentMode !== "INSTANT_RATE"} checked={serviceForm.instantPricingAvailable} onChange={event => setServiceForm(value => ({ ...value, instantPricingAvailable: event.target.checked }))} /><span>Instant pricing available</span></label></div><button className={styles.primary} disabled={busy}>{busy ? "Saving…" : "Save service"}</button></form>}
          {capabilities.canActivate && <div className={styles.launchActions}>{(selected.status === "DRAFT" || selected.status === "SUSPENDED") && <button className={styles.secondary} disabled={busy || !canMarkLocationReady} title={canMarkLocationReady ? "Mark this location ready" : "At least one Ready service must meet its verified-vendor requirement"} onClick={() => void changeStatus("MARK_READY")}>Mark ready</button>}{selected.status === "READY" && <button className={styles.primary} disabled={busy} onClick={() => void changeStatus("ACTIVATE")}>Activate location</button>}{(selected.status === "READY" || selected.status === "ACTIVE") && <button className={styles.danger} disabled={busy} onClick={() => void changeStatus("SUSPEND")}>Suspend</button>}</div>}
        </div>}
      </section>
    </div>
  </main>;
