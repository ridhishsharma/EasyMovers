...
import { ServiceLocationError, verifyExistingServiceLocation } from "@/lib/service-location-management";
...
export async function POST(request: Request, context: { params: Promise<{ locationId: string }> }) {
  ...
  try {
    ...
    const location = await verifyExistingServiceLocation(locationId, await request.json(), access.userId, request.headers.get("x-forwarded-for")?.split(",")[0]?.trim());
    ...
    return reply({ success: false, error: { code: "LOCATION_VERIFICATION_FAILED", message: "The existing location could not be verified." } }, 503);
  }
}
